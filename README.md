...
> Все эндпоинты требуют JWT. Данные возвращаются только по текущему пользователю.

### `GET /analytics`

Общая аналитика: доходы, расходы, баланс за период + процентные изменения по сравнению с предыдущим аналогичным периодом.

**Query:**
| Параметр | Тип | Описание |
|----------|-----|----------|
| `period` | `"day" \| "month" \| "year"` | Если не указан — за всё время (без процентных изменений) |
...
```json
{
  ...
  "totalExpense": 4300,
  "netBalance": 8200,
  "totalIncomePercent": 25.5,
  "totalExpensePercent": -10.25,
  "netBalancePercent": -15.75,
  "profitPercent": -15.75,
  "transactions": [ ... ]
}
```

- `*Percent` поля присутствуют только при указании `period`
- `profitPercent` — алиас `netBalancePercent` для обратной совместимости

---

### `GET /analytics/category`

Аналитика по конкретной категории за произвольный диапазон дат.

**Query:**
| Параметр | Тип | Обязательно | Описание |
|----------|-----|-------------|----------|
| `categoryId` | `number` | ✓ | ID категории |
| `startDate` | ISO 8601 | ✓ | Начало диапазона |
| `endDate` | ISO 8601 | ✓ | Конец диапазона |
...
Топ категорий по сумме транзакций (по убыванию).
...
```json
{
  "data": [
    {
      "category": { "id": 1, "title": "Food", "transactionType": "expense" },
      "totalAmount": 500
    }
  ]
}
